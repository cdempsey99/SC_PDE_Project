# A Traditional PINN i.e. we don't separate out the boundary conditions
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # Forward pass
        x = torch.relu(self.hidden1(x)) # Apply activation function to first layer
        x = torch.relu(self.hidden2(x)) # and second
        x = torch.relu(self.hidden3(x)) # and third
...
def trial_solution(x, y, model_output):
    ...
    return A(x, y) + x * (1 - x) * y * (1 - y) * (model_output)
